...
class RecipeTestCase(TestCase):
    def setUp(self):
        self.user_a = User.objects.create_user('cfe', password='abc123')
        self.recipe_a = Recipe.objects.create(name='Grilled chiken', user = self.user_a)
        self.recipe_b = Recipe.objects.create(name='Grilled chiken', user = self.user_a)
    ...
    def test_recipe_ingredient_forward_count(self):
        recipe = self.recipe_a
        qs = RecipeIngredient.objects.filter(recipe=recipe).all()
        self.assertEqual(qs.count(), 1)
    ...
    def test_user_two_level_relation_reverse(self):
        user = self.user_a
        recipeIngredients_ids = user.recipe_set.all().values_list('recipeingredient__id', flat=True)
        #print(recipeIngredients_ids)
    ...
    def test_unit_measure_validation(self):
        ...
        with self.assertRaises(ValidationError):
            for invalid_unit in invalid_units:
                ...
                ingredient.full_clean() #!IS_VALID()
